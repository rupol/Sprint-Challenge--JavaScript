...
console.log(`Tyrannosaurus weight: ${tRex.weight}`);

// What was the diet of a velociraptor?
console.log(`Velociraptor diet: ${raptor.diet}`);

// How long was a stegosaurus?
console.log(`Stegosaurus length: ${stego.length}`);

// What time period did tyrannosaurus live in?
console.log(`Tyrannosaurus time period: ${tRex.period}`);
...
const contactInfo = graduates.map(school => {
  return `${school.first_name} ${school.email}`;
});
console.log(contactInfo);

/* Request 3: Find out how many universities have the string "Uni" included in their name. Create a new array called uni that contains them all. Log the result. */
const uni = graduates.filter(contains => {
  return contains.university.includes("Uni");
});
console.log(uni);
